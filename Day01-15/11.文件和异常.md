...
可以将可能出错得代码放入到try语句，这样如果代码没有错误，则会正常执行。

如果出现错误，则会执行except中得内容 
...
fn()
#   File "c:/Users/tony5/Desktop/python/列表的介绍.py", line 1437, in <module>
#     fn()
#   File "c:/Users/tony5/Desktop/python/列表的介绍.py", line 1435, in fn      
#     print(10/0)
```

当异常在函数中出现时，如果在函数中对异常进行了处理，则异常部分继续传播

如果函数调用处处理了异常者不再继续传播，如果没有则继续向调用处传播
...
当程序运行过程中出现异常以后，所有异常信息都会被保存到一个专门的异常对象中，

而异常传播时，实际上就是异常对象抛给了调用处

比如：ZeroDivisionError类对象就是专门用来处理除以0的异常
